...
window.isMobile = function(){ // Copied from https://xobyte.org/scripts/isMobile.js
...
customElements.define('sky-cards', class extends HTMLElement {
    connectedCallback() {
        ...
        if(window.isMobile()) {
            ...
            popupText = popupText + `
                        <li class="sc-text-outline">Open SkyCards™ on your mobile device</li>
            `
        }
        popupText = popupText + `
...
//the same but with compacter text
customElements.define('sky-cards-compact', class extends HTMLElement {
    connectedCallback() {
        const userName = this.getAttribute('name') || 'Unknown User';
        const userCode = this.getAttribute('code') || '0000 0000 0000';
        const modalId = 'sc-modal-' + Math.random().toString(36).substr(2, 9);
